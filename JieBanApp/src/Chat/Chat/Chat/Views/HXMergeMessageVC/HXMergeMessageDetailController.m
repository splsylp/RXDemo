...
@interface HXMergeMessageDetailController ()<UITableViewDelegate,UITableViewDataSource,DocumentDownloadViewDelegate,UIActionSheetDelegate>
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([cell.model.merge_account isEqualToString:lastModel.merge_account]) {
        cell.mHeaderImageView.hidden = YES;
    }
    else {
...
//        [self cardShareCardTap:model];
        ...
        NSDictionary *domain = model.merge_userData.coverToDictionary;
    ...
    if (indexRow != NSNotFound) {
        ...
        for(NSDictionary *imgDic in imageArray){
            ...
            if([showCellImageDic.allKeys containsObject:imgDic[@"messageId"]]){
                ...
                CGRect imgRect = [showimgCell.mBubbleView.superview convertRect:showimgCell.mBubbleView.frame toView:[UIApplication sharedApplication].keyWindow.rootViewController.view];
                yxpModel.smallimageViewFrame = imgRect;
                yxpModel.smallImageView = ((HXMergerImageBubbleView *)showimgCell.mBubbleView).mImageView;
            }
...
- (void)cardShareCardTap:(HXMergeMessageModel *)model{
    NSDictionary *useDataDic = [MessageTypeManager getCusDicWithUserData:model.merge_userData];
