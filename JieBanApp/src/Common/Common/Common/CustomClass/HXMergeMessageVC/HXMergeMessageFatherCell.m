...
+ (NSString *)cellIdentifierForMessageModel:(HXMergeMessageModel *)model{
    ...
    if (im_jsonDic.allKeys.count == 0 && model.merge_userData.length > 0) { //可能是base64
        NSString *nobase64 = [HXMessageMergeManager getUserBase64DataString:model.merge_userData];
        im_jsonDic = [HXMessageMergeManager jsonDicWithNOBase64UserData:nobase64];
    }
...
- (NSDictionary *)getModelUsetData:(NSString *)userData {
    NSDictionary *im_jsonDic = [HXMessageMergeManager jsonDicWithNOBase64UserData:userData];
    if (im_jsonDic.allKeys.count == 0 && userData.length > 0) { //可能是base64
        NSString *nobase64 = [HXMessageMergeManager getUserBase64DataString:userData];
        im_jsonDic = [HXMessageMergeManager jsonDicWithNOBase64UserData:nobase64];
    }
    return im_jsonDic;
}
...
- (HXMergerMessageBubbleFatherView *)bubbleViewForMessageModel:(HXMergeMessageModel *)model{
    NSDictionary *im_jsonDic = [self getModelUsetData:model.merge_userData];
    if(model.merge_type.integerValue == MessageBodyType_Text){
        if(im_jsonDic[ShareCardMode] ||
           [im_jsonDic[SMSGTYPE] isEqualToString:TYPE_CARD]){
    ...
    }else if(model.merge_type.integerValue ==    MessageBodyType_File|| model.merge_type.integerValue ==    MessageBodyType_Call){
...
//                    if ([Common.sharedInstance.getAppid isEqualToString:dic[@"appid"]]) {
//                        voipinfo = dic;
//                    }
//                }
                //入库
