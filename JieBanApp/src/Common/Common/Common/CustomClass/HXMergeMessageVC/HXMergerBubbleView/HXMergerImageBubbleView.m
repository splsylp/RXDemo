...
//        _model = model;
        ...
        if(fileDic.allKeys.count == 0){
            ...
            [self.mImageView sd_setImageWithURL:[NSURL URLWithString:fileDic[@"fileUrl"]] completed:^(UIImage * _Nullable image, NSError * _Nullable error, SDImageCacheType cacheType, NSURL * _Nullable imageURL) {
                if (image) {
        ...
        if(fileDic.allKeys.count == 0){
            ...
            if ([[NSFileManager defaultManager] fileExistsAtPath:filePath]) {
                [self.mImageView sd_setImageWithURL:[NSURL fileURLWithPath:filePath] placeholderImage:ThemeImage(@"chat_placeholder_image") completed:^(UIImage * _Nullable image, NSError * _Nullable error, SDImageCacheType cacheType, NSURL * _Nullable imageURL) {
                    if (image) {
                        CGSize imageSize = image.size;
...
//            }
            ...
            [UIImageView.new sd_setImageWithURL:[NSURL URLWithString:fileDic[@"fileUrl"]] completed:^(UIImage * _Nullable image, NSError * _Nullable error, SDImageCacheType cacheType, NSURL * _Nullable imageURL) {
                CGSize imageSize = image.size;
