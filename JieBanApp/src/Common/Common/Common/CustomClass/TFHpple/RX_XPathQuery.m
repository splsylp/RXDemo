...
NSDictionary *DictionaryForNode(xmlNodePtr currentNode, NSMutableDictionary *parentResult,BOOL parentContent)
{
    ...
    if (nodeContent != NULL) {
        NSString *currentNodeContent = [NSString stringWithCString:(const char *)nodeContent
                                                          encoding:NSUTF8StringEncoding];
        currentNodeContent =[currentNodeContent stringByRemovingPercentEncoding];
    ...
    if (rawContent != nil) {
        resultForNode[@"raw"] = [rawContent stringByRemovingPercentEncoding];
    }
...
NSArray *PerformXPathQuery(xmlDocPtr doc, NSString *query)
{
    ...
    if(xpathCtx == NULL) {
        DDLogInfo(@"Unable to create XPath context.");
        return nil;
    }
    ...
    if(xpathObj == NULL) {
        DDLogInfo(@"Unable to evaluate XPath.");
