//
//  UILabel+RXAttributeTextTapAction.m
//
//  Created by LRX on 16/7/1.
//  Copyright © 2016年 LRX. All rights reserved.
//
...
@interface RXAttributeModel : NSObject
...
@implementation RXAttributeModel

@end

@implementation RXAttributeTextLabel
...
- (id<RXAttributeTapActionDelegate>)delegate
{
    return objc_getAssociatedObject(self, _cmd);
}

- (void)setDelegate:(id<RXAttributeTapActionDelegate>)delegate
{
...
- (void)RX_addAttributeTapActionWithStrings:(NSArray <NSString *> *)strings tapClicked:(void (^) (UILabel * label, NSString *string, NSRange range, NSInteger index))tapClick
{
    [self RX_removeAttributeTapActions];
    [self RX_getRangesWithStrings:strings];
    ...
    UITapGestureRecognizer *tap = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(rx_tap:)];
    [self addGestureRecognizer:tap];
}

- (void)RX_addAttributeTapActionWithStrings:(NSArray <NSString *> *)strings
                                   delegate:(id <RXAttributeTapActionDelegate> )delegate
{
    [self RX_removeAttributeTapActions];
    [self RX_getRangesWithStrings:strings];
    self.userInteractionEnabled = YES;
...
- (void)RX_addAttributeTapActionWithRanges:(NSArray<NSString *> *)ranges tapClicked:(void (^)(UILabel *, NSString *, NSRange, NSInteger))tapClick
{
    [self RX_removeAttributeTapActions];
    [self RX_getRangesWithRanges:ranges];
    self.userInteractionEnabled = YES;
...
- (void)RX_addAttributeTapActionWithRanges:(NSArray<NSString *> *)ranges delegate:(id<RXAttributeTapActionDelegate>)delegate
{
    [self RX_removeAttributeTapActions];
    [self RX_getRangesWithRanges:ranges];
    self.userInteractionEnabled = YES;
...
- (void)RX_removeAttributeTapActions
{
    self.tapBlock = nil;
    self.delegate = nil;
    self.effectDic = nil;
...
- (void)rx_tap:(UITapGestureRecognizer *)tap {
    if (!self.isTapAction) {
        return;
    }
    ...
    BOOL ret = [self RX_getTapFrameWithTouchPoint:point result:^(NSString *string, NSRange range, NSInteger index) {
        if (weakSelf.isTapEffect) {
            [weakSelf RX_saveEffectDicWithRange:range];
            [weakSelf RX_tapEffectWithStatus:YES];
        }
        ...
        if (tap.state == UIGestureRecognizerStateEnded) {
            ...
            if (weakSelf.delegate && [weakSelf.delegate respondsToSelector:@selector(RX_tapAttributeInLabel:string:range:index:)]) {
                [weakSelf.delegate RX_tapAttributeInLabel:weakSelf string:string range:range index:index];
            }
        }
    }];
    
    if (tap.state == UIGestureRecognizerStateEnded && ret) {
        if (self.isTapEffect) {
            [self performSelectorOnMainThread:@selector(RX_tapEffectWithStatus:) withObject:nil waitUntilDone:NO ];
        }
    }
...
- (BOOL)RX_getTapFrameWithTouchPoint:(CGPoint)point result:(void (^) (NSString *string , NSRange range , NSInteger index))resultBlock
{
    ...
    CGFloat total_height =  [self RX_textSizeWithAttributedString:self.attributedText width:self.bounds.size.width numberOfLines:0].height;
    ...
    CGAffineTransform transform = [self RX_transformForCoreText];
    ...
    for (CFIndex i = 0; i < count; i++) {
        ...
        CGRect flippedRect = [self RX_getLineBounds:line point:linePoint];
        
        CGRect rect = CGRectApplyAffineTransform(flippedRect, transform);
        
        CGFloat lineOutSpace = (self.bounds.size.height - total_height) / 2;
        
        rect.origin.y = lineOutSpace + [self RX_getLineOrign:line];
        ...
        if (CGRectContainsPoint(rect, point)) {
            ...
            for (int j = 0; j < link_count; j++) {
                ...
                RXAttributeModel *model = self.attributeStrings[j];
...
- (CGAffineTransform)RX_transformForCoreText
{
    return CGAffineTransformScale(CGAffineTransformMakeTranslation(0, self.bounds.size.height), 1.f, -1.f);
}

- (CGRect)RX_getLineBounds:(CTLineRef)line point:(CGPoint)point
{
    CGFloat ascent = 0.0f;
    ...
    height = [self RX_textSizeWithAttributedString:attributedString width:self.bounds.size.width numberOfLines:0].height;
    return CGRectMake(point.x, point.y , width, height);
}

- (CGFloat)RX_getLineOrign:(CTLineRef)line
{
    CFRange range = CTLineGetStringRange(line);
    if (range.location == 0) {
        return 0.;
        ...
        return [self RX_textSizeWithAttributedString:attributedString width:self.bounds.size.width numberOfLines:0].height;
    }
}

- (CGSize)RX_textSizeWithAttributedString:(NSAttributedString *)attributedString width:(float)width numberOfLines:(NSInteger)numberOfLines
{
    @autoreleasepool {
...
- (void)RX_tapEffectWithStatus:(BOOL)status
{
    if (self.isTapEffect) {
...
- (void)RX_saveEffectDicWithRange:(NSRange)range
{
...
- (void)RX_getRangesWithStrings:(NSArray <NSString *>  *)strings
{
    ...
    [strings enumerateObjectsUsingBlock:^(NSString * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if (range.length != 0) {
            ...
            totalStr = [totalStr stringByReplacingCharactersInRange:range withString:[weakSelf RX_getStringWithRange:range]];
            
            RXAttributeModel *model = [RXAttributeModel new];
            model.range = range;
            model.str = obj;
...
- (void)RX_getRangesWithRanges:(NSArray <NSString *>  *)ranges
{
    if (self.attributedText == nil) {
        self.isTapAction = NO;
        return;
...
- (NSString *)RX_getStringWithRange:(NSRange)range
{
