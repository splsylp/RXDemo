...
//
//  RX_MLSelectPhotoBrowserViewController.m
//  MLSelectPhoto
//
...
#import "UIImage+RX_MLTint.h"

// 分页控制器的高度
static NSInteger ZLPickerColletionViewPadding = 20;
static NSString *_cellIdentifier = @"collectionViewCell";

@interface RX_MLSelectPhotoBrowserViewController () <UIScrollViewDelegate,ZLPhotoPickerPhotoScrollViewDelegate,UICollectionViewDelegateFlowLayout,UICollectionViewDataSource,UICollectionViewDelegate>
...
@implementation RX_MLSelectPhotoBrowserViewController
...
- (UIButton *)doneBtn{
    if (!_doneBtn) {
        ...
        [rightBtn setTitle:languageStringWithKey(@"发送") forState:UIControlStateNormal];
...
- (UIButton *)deleleBtn{
    if (!_deleleBtn) {
        ...
        deleleBtn.titleLabel.font = ThemeFontLarge;
        [deleleBtn setImage:ThemeImage(MLSelectPhotoSrcName(@"AssetsPickerChecked")) forState:UIControlStateNormal];
...
- (void)viewDidLoad {
    ...
    self.navigationItem.leftBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:languageStringWithKey(@"返回") style:UIBarButtonItemStyleDone target:self action:@selector(popClicked)];
...
//        [self.deleleBtn setImage:[ThemeImage(MLSelectPhotoSrcName(@"AssetsPickerChecked")) imageWithTintColor:[UIColor grayColor]] forState:UIControlStateNormal];
        ...
        [self.deleleBtn setImage:ThemeImage(MLSelectPhotoSrcName(@"AssetsPickerChecked")) forState:UIControlStateNormal];
    }
...
- (UICollectionViewCell *) collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (self.photos.count) {
        ...
        RX_MLSelectPhotoAssets *photo = self.photos[indexPath.item]; //[self.dataSource photoBrowser:self photoAtIndex:indexPath.item];
        ...
        RX_MLSelectPhotoPickerBrowserPhotoScrollView *scrollView =  [[RX_MLSelectPhotoPickerBrowserPhotoScrollView alloc] init];
...
- (void)setPageLabelPage:(NSInteger)page{
    self.title = [NSString stringWithFormat:@"%d/ %d",(int)(page + 1), (int)self.photos.count];
}
