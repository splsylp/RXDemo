...
@implementation RX_MLSelectPhotoPickerBrowserPhoto

- (void)setPhotoObj:(id)photoObj{
    _photoObj = photoObj;
    
    if ([photoObj isKindOfClass:[RX_MLSelectPhotoAssets class]]) {
        RX_MLSelectPhotoAssets *asset = (RX_MLSelectPhotoAssets *)photoObj;
        self.asset = asset;
...
+ (instancetype)photoAnyImageObjWith:(id)imageObj{
    RX_MLSelectPhotoPickerBrowserPhoto *photo = [[self alloc] init];
