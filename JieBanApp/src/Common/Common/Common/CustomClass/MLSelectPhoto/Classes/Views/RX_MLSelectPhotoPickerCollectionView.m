...
#import "RX_MLPhotoPickerImageView.h"
#import "RX_MLSelectPhotoPickerFooterCollectionReusableView.h"
#import "RX_MLSelectPhotoCommon.h"

@interface RX_MLSelectPhotoPickerCollectionView () <UICollectionViewDataSource,UICollectionViewDelegate>

@property (nonatomic , strong) RX_MLSelectPhotoPickerFooterCollectionReusableView *footerView;
...
@implementation RX_MLSelectPhotoPickerCollectionView
...
- (void)setDataArray:(NSArray *)dataArray{
    ...
    if (self.isRecoderSelectPicker){
        ...
        for (RX_MLSelectPhotoAssets *asset in self.selectAsstes) {
            for (RX_MLSelectPhotoAssets *asset2 in self.dataArray) {
...
- (void)setSelectAsstes:(NSMutableArray *)selectAsstes{
    _selectAsstes = selectAsstes;
    _selectsIndexPath = [[NSMutableArray alloc] init];
    for (RX_MLSelectPhotoAssets *asset in _selectAsstes) {
        for (RX_MLSelectPhotoAssets *asset2 in self.dataArray) {
            if ([asset.asset.defaultRepresentation.url isEqual:asset2.asset.defaultRepresentation.url]) {
                NSInteger index = [self.dataArray indexOfObject:asset2];
                [_selectsIndexPath addObject:@(index)];
            }
        }
    }
    [self reloadData];
...
- (UICollectionViewCell *) collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    RX_MLSelectPhotoPickerCollectionViewCell *cell = [RX_MLSelectPhotoPickerCollectionViewCell cellWithCollectionView:collectionView cellForItemAtIndexPath:indexPath];
    
    RX_MLPhotoPickerImageView *cellImgView = [[RX_MLPhotoPickerImageView alloc] initWithFrame:cell.bounds];
    ...
    if(indexPath.item == 0 && self.topShowPhotoPicker){
        ...
        imageView.image = ThemeImage(MLSelectPhotoSrcName(@"camera"));
    }else{
        
        // 需要记录选中的值的数据
        if (self.isRecoderSelectPicker) {
            for (RX_MLSelectPhotoAssets *asset in self.selectAsstes) {
                if ([asset.asset.defaultRepresentation.url isEqual:[(RX_MLSelectPhotoAssets *)self.dataArray[indexPath.item] asset].defaultRepresentation.url]) {
        ...
        RX_MLSelectPhotoAssets *asset = self.dataArray[indexPath.item];
        cellImgView.isVideoType = asset.isVideoType;
        if ([asset isKindOfClass:[RX_MLSelectPhotoAssets class]]) {
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    RX_MLSelectPhotoPickerCollectionViewCell *cell = (RX_MLSelectPhotoPickerCollectionViewCell *) [collectionView cellForItemAtIndexPath:indexPath];
    
    RX_MLSelectPhotoAssets *asset = self.dataArray[indexPath.row];
    RX_MLPhotoPickerImageView *pickerImageView = [cell.contentView.subviews lastObject];
    // 如果没有就添加到数组里面，存在就移除
    if (pickerImageView.isMaskViewFlag) {
        ...
        if (self.selectAsstes.count >= minCount) {
            ...
            NSString *format = [NSString stringWithFormat:@"%@%zd%@",languageStringWithKey(@"最多只能选择"),minCount,languageStringWithKey(@"张图片")];
            if (minCount == 0) {
                format = [NSString stringWithFormat:@"%@",languageStringWithKey(@"您已经选满了图片")];
            }
            UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:languageStringWithKey(@"提醒") message:format delegate:self cancelButtonTitle:nil otherButtonTitles:languageStringWithKey(@"好的"), nil];
            [alertView show];
            return ;
        }
    ...
    pickerImageView.maskViewFlag = ([pickerImageView isKindOfClass:[RX_MLPhotoPickerImageView class]]) && !pickerImageView.isMaskViewFlag;
...
{
    RX_MLSelectPhotoPickerFooterCollectionReusableView *reusableView = nil;
    if (kind == UICollectionElementKindSectionFooter) {
        RX_MLSelectPhotoPickerFooterCollectionReusableView *footerView = [collectionView dequeueReusableSupplementaryViewOfKind:UICollectionElementKindSectionFooter withReuseIdentifier:@"FooterView" forIndexPath:indexPath];
        footerView.count = self.dataArray.count;
        reusableView = footerView;
        self.footerView = footerView;
    }
    return reusableView?:[[UICollectionReusableView alloc] init];
}
