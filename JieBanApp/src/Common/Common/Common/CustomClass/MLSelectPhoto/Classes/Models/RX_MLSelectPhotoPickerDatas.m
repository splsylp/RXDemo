...
@interface RX_MLSelectPhotoPickerDatas ()
@property (nonatomic , strong) ALAssetsLibrary *library;
@end

@implementation RX_MLSelectPhotoPickerDatas
...
- (void) getAllGroupAllPhotos2:(BOOL)allPhotos withResource : (callBackBlock ) callBack{
    ...
    ALAssetsLibraryGroupsEnumerationResultsBlock resultBlock = ^(ALAssetsGroup *group, BOOL *stop){
        if (group) {
            ...
            // 包装一个模型来赋值
            RX_MLSelectPhotoPickerGroup *pickerGroup = [[RX_MLSelectPhotoPickerGroup alloc] init];
