...
@implementation RX_TZImageCropManager
...
+ (UIImage *)cropImageView:(UIImageView *)imageView toRect:(CGRect)rect zoomScale:(double)zoomScale containerView:(UIView *)containerView {
    ...
    cropedImage = [[RX_TZImageManager manager] fixOrientation:cropedImage];
...
+ (UIImage *)sd_tz_animatedGIFWithData:(NSData *)data {
    ...
    if (count <= 1) {
        ...
        NSInteger maxCount = [RX_TZImagePickerConfig sharedInstance].gifPreviewMaxImagesCount ?: 200;
