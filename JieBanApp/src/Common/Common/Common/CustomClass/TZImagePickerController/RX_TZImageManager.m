...
@interface RX_TZImageManager ()
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
@end

@implementation RX_TZImageManager

CGSize RX_AssetGridThumbnailSize;
CGFloat RX_TZScreenWidth;
CGFloat RX_TZScreenScale;

static RX_TZImageManager *manager;
...
+ (instancetype)manager {
    dispatch_once(&onceToken, ^{
        ...
        [manager configRX_TZScreenWidth];
    });
    return manager;
}
...
- (void)setPhotoWidth:(CGFloat)photoWidth {
    ...
    RX_TZScreenWidth = photoWidth / 2;
}

- (void)setColumnNumber:(NSInteger)columnNumber {
    [self configRX_TZScreenWidth];

    _columnNumber = columnNumber;
    CGFloat margin = 4;
    CGFloat itemWH = (RX_TZScreenWidth - 2 * margin - 4) / columnNumber - margin;
    RX_AssetGridThumbnailSize = CGSizeMake(itemWH * RX_TZScreenScale, itemWH * RX_TZScreenScale);
}

- (void)configRX_TZScreenWidth {
    RX_TZScreenWidth = [UIScreen mainScreen].bounds.size.width;
    // 测试发现，如果scale在plus真机上取到3.0，内存会增大特别多。故这里写死成2.0
    RX_TZScreenScale = 2.0;
    if (RX_TZScreenWidth > 700) {
        RX_TZScreenScale = 1.5;
    }
}
...
/// Get Album 获得相册/相册数组
- (void)getCameraRollAlbum:(BOOL)allowPickingVideo allowPickingImage:(BOOL)allowPickingImage needFetchAssets:(BOOL)needFetchAssets completion:(void (^)(RX_TZAlbumModel *model))completion {
    __block RX_TZAlbumModel *model;
...
- (void)getAllAlbums:(BOOL)allowPickingVideo allowPickingImage:(BOOL)allowPickingImage needFetchAssets:(BOOL)needFetchAssets completion:(void (^)(NSArray<RX_TZAlbumModel *> *))completion{
...
/// Get Assets 获得照片数组
- (void)getAssetsFromFetchResult:(PHFetchResult *)result completion:(void (^)(NSArray<RX_TZAssetModel *> *))completion {
    RX_TZImagePickerConfig *config = [RX_TZImagePickerConfig sharedInstance];
...
- (void)getAssetsFromFetchResult:(PHFetchResult *)result allowPickingVideo:(BOOL)allowPickingVideo allowPickingImage:(BOOL)allowPickingImage completion:(void (^)(NSArray<RX_TZAssetModel *> *))completion {
    NSMutableArray *photoArr = [NSMutableArray array];
    [result enumerateObjectsUsingBlock:^(PHAsset *asset, NSUInteger idx, BOOL * _Nonnull stop) {
        RX_TZAssetModel *model = [self assetModelWithAsset:asset allowPickingVideo:allowPickingVideo allowPickingImage:allowPickingImage];
        if (model) {
...
- (void)getAssetFromFetchResult:(PHFetchResult *)result atIndex:(NSInteger)index allowPickingVideo:(BOOL)allowPickingVideo allowPickingImage:(BOOL)allowPickingImage completion:(void (^)(RX_TZAssetModel *))completion {
    PHAsset *asset;
    @try {
        asset = result[index];
    }
...
- (RX_TZAssetModel *)assetModelWithAsset:(PHAsset *)asset allowPickingVideo:(BOOL)allowPickingVideo allowPickingImage:(BOOL)allowPickingImage {
    BOOL canSelect = YES;
    ...
    model = [RX_TZAssetModel modelWithAsset:asset type:type timeLength:timeLength];
    return model;
}
...
- (void)getPhotosBytesWithArray:(NSArray *)photos completion:(void (^)(NSString *totalBytes))completion {
    ...
    for (NSInteger i = 0; i < photos.count; i++) {
        RX_TZAssetModel *model = photos[i];
...
- (int32_t)getPhotoWithAsset:(PHAsset *)asset completion:(void (^)(UIImage *, NSDictionary *, BOOL isDegraded))completion {
    CGFloat fullScreenWidth = RX_TZScreenWidth;
...
- (int32_t)getPhotoWithAsset:(PHAsset *)asset photoWidth:(CGFloat)photoWidth completion:(void (^)(UIImage *photo,NSDictionary *info,BOOL isDegraded))completion progressHandler:(void (^)(double progress, NSError *error, BOOL *stop, NSDictionary *info))progressHandler networkAccessAllowed:(BOOL)networkAccessAllowed {
    CGSize imageSize;
    if (photoWidth < RX_TZScreenWidth && photoWidth < _photoPreviewMaxWidth) {
        imageSize = RX_AssetGridThumbnailSize;
    } else {
        PHAsset *phAsset = (PHAsset *)asset;
        CGFloat aspectRatio = phAsset.pixelWidth / (CGFloat)phAsset.pixelHeight;
        CGFloat pixelWidth = photoWidth * RX_TZScreenScale * 1.5;
        // 超宽图片
        if (aspectRatio > 1.8) {
...
/// Get postImage / 获取封面图
- (void)getPostImageWithAlbumModel:(RX_TZAlbumModel *)model completion:(void (^)(UIImage *))completion {
    id asset = [model.rxresult lastObject];
    if (!self.sortAscendingByModificationDate) {
        asset = [model.rxresult firstObject];
    }
    [[RX_TZImageManager manager] getPhotoWithAsset:asset photoWidth:80 completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
...
- (void)startExportVideoWithVideoAsset:(AVURLAsset *)videoAsset presetName:(NSString *)presetName success:(void (^)(NSString *outputPath))success failure:(void (^)(NSString *errorMessage, NSError *error))failure {
    ...
    if ([presets containsObject:presetName]) {
        ...
        if ([RX_TZImagePickerConfig sharedInstance].needFixComposition) {
...
- (RX_TZAlbumModel *)modelWithResult:(PHFetchResult *)result name:(NSString *)name isCameraRoll:(BOOL)isCameraRoll needFetchAssets:(BOOL)needFetchAssets {
    RX_TZAlbumModel *model = [[RX_TZAlbumModel alloc] init];
    [model rxsetResult:result needFetchAssets:needFetchAssets];
    model.rxname = name;
    model.rxisCameraRoll = isCameraRoll;
    model.rxcount = result.count;
    return model;
}
...
- (RX_TZAssetModel *)createModelWithAsset:(PHAsset *)asset {
    TZAssetModelMediaType type = [[RX_TZImageManager manager] getAssetType:asset];
    NSString *timeLength = type == TZAssetModelMediaTypeVideo ? [NSString stringWithFormat:@"%0.0f",asset.duration] : @"";
    timeLength = [[RX_TZImageManager manager] getNewTimeFromDurationSecond:timeLength.integerValue];
    RX_TZAssetModel *model = [RX_TZAssetModel modelWithAsset:asset type:type timeLength:timeLength];
    return model;
}
...
//    return [NSNumber numberWithBool:![RX_TZImageManager manager].sortAscendingByModificationDate];
//}
//
//@end
