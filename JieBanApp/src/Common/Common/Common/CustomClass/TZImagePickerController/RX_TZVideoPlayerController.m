...
@interface RX_TZVideoPlayerController () {
    AVPlayer *_player;
...
@implementation RX_TZVideoPlayerController
...
- (void)configMoviePlayer {
    [[RX_TZImageManager manager] getPhotoWithAsset:_model.asset completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
        if (!isDegraded && photo) {
            self->_cover = photo;
            self->_doneButton.enabled = YES;
        }
    }];
    [[RX_TZImageManager manager] getVideoWithAsset:_model.asset completion:^(AVPlayerItem *playerItem, NSDictionary *info) {
...
- (void)configBottomToolBar {
    ...
    _doneButton.titleLabel.font = ThemeFontLarge;
    if (!_cover) {
