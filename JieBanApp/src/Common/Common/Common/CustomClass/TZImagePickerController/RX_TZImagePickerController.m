//
//  RX_TZImagePickerController.m
//  RX_TZImagePickerController
//
//  Created by 谭真 on 15/12/24.
//  Copyright © 2015年 谭真. All rights reserved.
//  version 3.0.5 - 2018.09.08
//  更多信息，请前往项目的github地址：https://github.com/banchichen/RX_TZImagePickerController
...
@interface RX_TZImagePickerController () {
    NSTimer *_timer;
    UILabel *_tipLabel;
...
/// Default is 4, Use in photos collectionView in RX_TZPhotoPickerController
/// 默认4列, RX_TZPhotoPickerController中的照片collectionView
@property (nonatomic, assign) NSInteger columnNumber;
@end

@implementation RX_TZImagePickerController
...
- (void)viewDidLoad {
    ...
    [RX_TZImageManager manager].shouldFixOrientation = NO;
    ...
    [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(cancelButtonClick) name:@"TZImagePickerVCcancelButtonClick" object:nil];
}
...
- (void)configBarButtonItemAppearance {
    ...
    if (@available(iOS 9, *)) {
        barItem = [UIBarButtonItem appearanceWhenContainedInInstancesOfClasses:@[[RX_TZImagePickerController class]]];
    } else {
        barItem = [UIBarButtonItem appearanceWhenContainedIn:[RX_TZImagePickerController class], nil];
    }
    NSMutableDictionary *textAttrs = [NSMutableDictionary dictionary];
...
- (instancetype)initWithMaxImagesCount:(NSInteger)maxImagesCount delegate:(id<RX_TZImagePickerControllerDelegate>)delegate {
    return [self initWithMaxImagesCount:maxImagesCount columnNumber:4 delegate:delegate pushPhotoPickerVc:YES];
}

- (instancetype)initWithMaxImagesCount:(NSInteger)maxImagesCount columnNumber:(NSInteger)columnNumber delegate:(id<RX_TZImagePickerControllerDelegate>)delegate {
    return [self initWithMaxImagesCount:maxImagesCount columnNumber:columnNumber delegate:delegate pushPhotoPickerVc:YES];
}

- (instancetype)initWithMaxImagesCount:(NSInteger)maxImagesCount columnNumber:(NSInteger)columnNumber delegate:(id<RX_TZImagePickerControllerDelegate>)delegate pushPhotoPickerVc:(BOOL)pushPhotoPickerVc {
    _pushPhotoPickerVc = pushPhotoPickerVc;
    RX_TZAlbumPickerController *albumPickerVc = [[RX_TZAlbumPickerController alloc] init];
    ...
    if (self) {
        ...
        if (![[RX_TZImageManager manager] authorizationStatusAuthorized]) {
            _tipLabel = [[UILabel alloc] init];
            _tipLabel.frame = CGRectMake(8, 120, self.view.tz_width - 16, 60);
            _tipLabel.textAlignment = NSTextAlignmentCenter;
            _tipLabel.numberOfLines = 0;
            _tipLabel.font = SystemFontLarge;
            ...
            _settingBtn.titleLabel.font = SystemFontLarge;
...
- (instancetype)initWithSelectedAssets:(NSMutableArray *)selectedAssets selectedPhotos:(NSMutableArray *)selectedPhotos index:(NSInteger)index{
    RX_TZPhotoPreviewController *previewVc = [[RX_TZPhotoPreviewController alloc] init];
...
- (void)configDefaultSetting {
    ...
    self.naviTitleFont = SystemFontLarge;
    self.barItemTextFont = SystemFontLarge;
...
- (void)setShowSelectedIndex:(BOOL)showSelectedIndex {
    ...
    [RX_TZImagePickerConfig sharedInstance].showSelectedIndex = showSelectedIndex;
}

- (void)setShowPhotoCannotSelectLayer:(BOOL)showPhotoCannotSelectLayer {
    _showPhotoCannotSelectLayer = showPhotoCannotSelectLayer;
    [RX_TZImagePickerConfig sharedInstance].showPhotoCannotSelectLayer = showPhotoCannotSelectLayer;
}

- (void)setNotScaleImage:(BOOL)notScaleImage {
    _notScaleImage = notScaleImage;
    [RX_TZImagePickerConfig sharedInstance].notScaleImage = notScaleImage;
}

- (void)setNeedFixComposition:(BOOL)needFixComposition {
    _needFixComposition = needFixComposition;
    [RX_TZImagePickerConfig sharedInstance].needFixComposition = needFixComposition;
}
...
- (void)observeAuthrizationStatusChange {
    ...
    if ([[RX_TZImageManager manager] authorizationStatusAuthorized]) {
        ...
        RX_TZAlbumPickerController *albumPickerVc = (RX_TZAlbumPickerController *)self.visibleViewController;
        if ([albumPickerVc isKindOfClass:[RX_TZAlbumPickerController class]]) {
...
- (void)pushPhotoPickerVc {
    ...
    if (!_didPushPhotoPickerVc && _pushPhotoPickerVc) {
        RX_TZPhotoPickerController *photoPickerVc = [[RX_TZPhotoPickerController alloc] init];
        photoPickerVc.isFirstAppear = YES;
        photoPickerVc.columnNumber = self.columnNumber;
        [[RX_TZImageManager manager] getCameraRollAlbum:self.allowPickingVideo allowPickingImage:self.allowPickingImage needFetchAssets:NO completion:^(RX_TZAlbumModel *model) {
...
- (void)showProgressHUD {
    if (!_progressHUD) {
        ...
        _HUDLabel.font = SystemFontLarge;
...
- (void)setPickerDelegate:(id<RX_TZImagePickerControllerDelegate>)pickerDelegate {
    _pickerDelegate = pickerDelegate;
    [RX_TZImageManager manager].pickerDelegate = pickerDelegate;
}
...
- (void)setPhotoPreviewMaxWidth:(CGFloat)photoPreviewMaxWidth {
    ...
    [RX_TZImageManager manager].photoPreviewMaxWidth = _photoPreviewMaxWidth;
}

- (void)setPhotoWidth:(CGFloat)photoWidth {
    _photoWidth = photoWidth;
    [RX_TZImageManager manager].photoWidth = photoWidth;
}
...
- (void)setSelectedAssets:(NSMutableArray *)selectedAssets {
    ...
    for (PHAsset *asset in selectedAssets) {
        RX_TZAssetModel *model = [RX_TZAssetModel modelWithAsset:asset type:[[RX_TZImageManager manager] getAssetType:asset]];
...
- (void)setAllowPickingImage:(BOOL)allowPickingImage {
    ...
    [RX_TZImagePickerConfig sharedInstance].allowPickingImage = allowPickingImage;
...
- (void)setAllowPickingVideo:(BOOL)allowPickingVideo {
    ...
    [RX_TZImagePickerConfig sharedInstance].allowPickingVideo = allowPickingVideo;
...
- (void)setPreferredLanguage:(NSString *)preferredLanguage {
    ...
    [RX_TZImagePickerConfig sharedInstance].preferredLanguage = preferredLanguage;
    [self configDefaultBtnTitle];
}

- (void)setLanguageBundle:(NSBundle *)languageBundle {
    _languageBundle = languageBundle;
    [RX_TZImagePickerConfig sharedInstance].languageBundle = languageBundle;
    [self configDefaultBtnTitle];
}
...
- (void)setSortAscendingByModificationDate:(BOOL)sortAscendingByModificationDate {
    ...
    [RX_TZImageManager manager].sortAscendingByModificationDate = sortAscendingByModificationDate;
}
...
- (void)addSelectedModel:(RX_TZAssetModel *)model {
    [_selectedModels addObject:model];
    [_selectedAssetIds addObject:model.asset.localIdentifier];
}

- (void)removeSelectedModel:(RX_TZAssetModel *)model {
...
@interface RX_TZAlbumPickerController ()<UITableViewDataSource,UITableViewDelegate> {
    UITableView *_tableView;
}
@property (nonatomic, strong) NSMutableArray *albumArr;
@end

@implementation RX_TZAlbumPickerController
...
- (void)viewDidLoad {
    ...
    CGSize size = [imagePickerVc.cancelBtnTitleStr sizeWithFont:SystemFontLarge];
    CGRect btnFrame = CGRectMake(0, 0, size.width + 20, 30);
    UIButton *button = [UIButton buttonWithType:UIButtonTypeCustom];
    [button setFrame:btnFrame];
    [button setTitle:imagePickerVc.cancelBtnTitleStr forState:UIControlStateNormal];
...
//    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        RX_TZImagePickerController *imagePickerVc = (RX_TZImagePickerController *)self.navigationController;
        [[RX_TZImageManager manager] getAllAlbums:imagePickerVc.allowPickingVideo allowPickingImage:imagePickerVc.allowPickingImage needFetchAssets:!self.isFirstAppear completion:^(NSArray<RX_TZAlbumModel *> *models) {
            dispatch_async(dispatch_get_main_queue(), ^{
                self->_albumArr = [NSMutableArray arrayWithArray:models];
                for (RX_TZAlbumModel *albumModel in self->_albumArr) {
                    albumModel.rxselectedModels = imagePickerVc.selectedModels;
                }
                [imagePickerVc hideProgressHUD];
                ...
                if (!self->_tableView) {
                    ...
                    [self->_tableView registerClass:[RX_TZAlbumCell class] forCellReuseIdentifier:@"RX_TZAlbumCell"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    RX_TZAlbumCell *cell = [tableView dequeueReusableCellWithIdentifier:@"RX_TZAlbumCell"];
    RX_TZImagePickerController *imagePickerVc = (RX_TZImagePickerController *)self.navigationController;
    cell.albumCellDidLayoutSubviewsBlock = imagePickerVc.albumCellDidLayoutSubviewsBlock;
    cell.rxalbumCellDidSetModelBlock = imagePickerVc.rxalbumCellDidSetModelBlock;
    cell.rxselectedCountButton.backgroundColor = imagePickerVc.iconThemeColor;
    cell.rxmodel = _albumArr[indexPath.row];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    RX_TZPhotoPickerController *photoPickerVc = [[RX_TZPhotoPickerController alloc] init];
    photoPickerVc.columnNumber = self.columnNumber;
    RX_TZAlbumModel *model = _albumArr[indexPath.row];
    photoPickerVc.model = model;
...
@implementation RX_TZCommonTools

+ (BOOL)rxtz_isIPhoneX {
    // eagle 这也是判断iPhoneX的方法
...
//    }
//    // iPhone10,6是美版iPhoneX 感谢hegelsu指出：https://github.com/banchichen/RX_TZImagePickerController/issues/635
//    BOOL isIPhoneX = [platform isEqualToString:@"iPhone10,3"] || [platform isEqualToString:@"iPhone10,6"];
    return isIPhoneX;
}

+ (CGFloat)rxtz_statusBarHeight {
    return [self rxtz_isIPhoneX] ? 44 : 20;
}

// 获得Info.plist数据字典
+ (NSDictionary *)rxtz_getInfoDictionary {
//    NSDictionary *infoDict = [NSBundle mainBundle].localizedInfoDictionary;
    NSDictionary *infoDict = [NSDictionary dictionary];
...
@implementation RX_TZImagePickerConfig

+ (instancetype)sharedInstance {
    static dispatch_once_t onceToken;
    static RX_TZImagePickerConfig *config = nil;
    dispatch_once(&onceToken, ^{
        if (config == nil) {
            config = [[RX_TZImagePickerConfig alloc] init];
