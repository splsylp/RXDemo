...
@interface RX_TZPhotoPreviewController ()<UICollectionViewDataSource,UICollectionViewDelegate,UIScrollViewDelegate> {
...
@implementation RX_TZPhotoPreviewController

- (void)viewDidLoad {
    [super viewDidLoad];
    [RX_TZImageManager manager].shouldFixOrientation = YES;
    RX_TZImagePickerController *_tzImagePickerVc = (RX_TZImagePickerController *)self.navigationController;
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    if (![[UIDevice currentDevice].systemVersion isEqualToString:@"13.1.3"] && ![[UIDevice currentDevice].systemVersion isEqualToString:@"13.3.1"]) {//13.1.3系统状态栏不正常
...
- (void)configCustomNaviBar {
    ...
    _indexLabel.font = SystemFontMiddle;
...
- (void)configBottomToolBar {
    ...
    if (_tzImagePickerVc.allowPickingOriginalPhoto) {
        ...
        _originalPhotoButton.titleLabel.font = SystemFontMiddle;
        ...
        _originalPhotoLabel.font = SystemFontMiddle;
...
- (void)configCollectionView {
    ...
    [_collectionView registerClass:[RX_TZPhotoPreviewCell class] forCellWithReuseIdentifier:@"RX_TZPhotoPreviewCell"];
    [_collectionView registerClass:[RX_TZVideoPreviewCell class] forCellWithReuseIdentifier:@"RX_TZVideoPreviewCell"];
    [_collectionView registerClass:[RX_TZGifPreviewCell class] forCellWithReuseIdentifier:@"RX_TZGifPreviewCell"];
}
...
- (void)configCropView {
    ...
    if (_tzImagePickerVc.maxImagesCount <= 1 && _tzImagePickerVc.allowCrop) {
        ...
        [RX_TZImageCropManager overlayClippingWithView:_cropBgView cropRect:_tzImagePickerVc.cropRect containerView:self.view needCircleCrop:_tzImagePickerVc.needCircleCrop];
...
- (void)viewDidLayoutSubviews {
    ...
    CGFloat statusBarHeight = [RX_TZCommonTools rxtz_statusBarHeight];
    ...
    if (_tzImagePickerVc.allowPickingOriginalPhoto) {
        CGFloat fullImageWidth = [_tzImagePickerVc.fullImageBtnTitleStr boundingRectWithSize:CGSizeMake(CGFLOAT_MAX, CGFLOAT_MAX) options:NSStringDrawingUsesFontLeading attributes:@{NSFontAttributeName:SystemFontMiddle} context:nil].size.width;
...
- (void)select:(UIButton *)selectButton {
    RX_TZImagePickerController *_tzImagePickerVc = (RX_TZImagePickerController *)self.navigationController;
    RX_TZAssetModel *model = _models[_currentIndex];
        ...
        for (RX_TZAssetModel *model_item in selectedModels) {
            if ([model.asset.localIdentifier isEqualToString:model_item.asset.localIdentifier]) {
                ...
                for (NSInteger i = 0; i < selectedModelsTmp.count; i++) {
                    RX_TZAssetModel *model = selectedModelsTmp[i];
    ...
    [self refreshNaviBarAndBottomBarState];
    if (model.isSelected) {
        [UIView showOscillatoryAnimationWithLayer:selectButton.imageView.layer type:RX_TZOscillatoryAnimationToBigger];
    }
    [UIView showOscillatoryAnimationWithLayer:_numberImageView.layer type:RX_TZOscillatoryAnimationToSmaller];
}

- (void)backButtonClick {
    if (self.navigationController.childViewControllers.count < 2) {
        [self.navigationController dismissViewControllerAnimated:YES completion:nil];
        if ([self.navigationController isKindOfClass: [RX_TZImagePickerController class]]) {
            RX_TZImagePickerController *nav = (RX_TZImagePickerController *)self.navigationController;
...
- (void)doneButtonClick {
    ...
    if (_tzImagePickerVc.allowCrop) { // 裁剪状态
        ...
        RX_TZPhotoPreviewCell *cell = (RX_TZPhotoPreviewCell *)[_collectionView cellForItemAtIndexPath:indexPath];
        UIImage *cropedImage = [RX_TZImageCropManager cropImageView:cell.rxpreviewView.imageView toRect:_tzImagePickerVc.cropRect zoomScale:cell.rxpreviewView.scrollView.zoomScale containerView:self.view];
        if (_tzImagePickerVc.needCircleCrop) {
            cropedImage = [RX_TZImageCropManager circularClipImage:cropedImage];
        }
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    RX_TZAssetPreviewCell *cell;
    __weak typeof(self) weakSelf = self;
    if (_tzImagePickerVc.allowPickingMultipleVideo && model.type == TZAssetModelMediaTypeVideo) {
        cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"RX_TZVideoPreviewCell" forIndexPath:indexPath];
        ...
        cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"RX_TZGifPreviewCell" forIndexPath:indexPath];
    } else {
        cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"RX_TZPhotoPreviewCell" forIndexPath:indexPath];
        RX_TZPhotoPreviewCell *photoPreviewCell = (RX_TZPhotoPreviewCell *)cell;
...
- (void)collectionView:(UICollectionView *)collectionView willDisplayCell:(UICollectionViewCell *)cell forItemAtIndexPath:(NSIndexPath *)indexPath {
    if ([cell isKindOfClass:[RX_TZPhotoPreviewCell class]]) {
        [(RX_TZPhotoPreviewCell *)cell recoverSubviews];
    }
}
...
- (void)collectionView:(UICollectionView *)collectionView didEndDisplayingCell:(UICollectionViewCell *)cell forItemAtIndexPath:(NSIndexPath *)indexPath {
    if ([cell isKindOfClass:[RX_TZPhotoPreviewCell class]]) {
        [(RX_TZPhotoPreviewCell *)cell recoverSubviews];
    } else if ([cell isKindOfClass:[RX_TZVideoPreviewCell class]]) {
        [(RX_TZVideoPreviewCell *)cell pausePlayerAndShowNaviBar];
    }
}
...
- (void)showPhotoBytes {
    [[RX_TZImageManager manager] getPhotosBytesWithArray:@[_models[_currentIndex]] completion:^(NSString *totalBytes) {
