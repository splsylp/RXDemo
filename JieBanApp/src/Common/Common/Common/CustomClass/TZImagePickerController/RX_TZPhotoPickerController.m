...
#import "RX_TZPhotoPickerController.h"
#import "RX_TZImagePickerController.h"
#import "RX_TZPhotoPreviewController.h"
#import "RX_TZAssetCell.h"
#import "RX_TZAssetModel.h"
#import "UIView+RX_Layout.h"
#import "RX_TZImageManager.h"
#import "RX_TZVideoPlayerController.h"
#import "RX_TZGifPhotoPreviewController.h"
#import "RX_TZLocationManager.h"
#import <MobileCoreServices/MobileCoreServices.h>

@interface RX_TZPhotoPickerController ()<UICollectionViewDataSource,UICollectionViewDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate,UIAlertViewDelegate> {
...
@property (nonatomic, strong) RX_TZCollectionView *collectionView;
...
static CGSize RX_AssetGridThumbnailSize;
static CGFloat itemMargin = 5;

@implementation RX_TZPhotoPickerController
...
- (UIImagePickerController *)imagePickerVc {
    if (_imagePickerVc == nil) {
        ...
        if (@available(iOS 9, *)) {
            tzBarItem = [UIBarButtonItem appearanceWhenContainedInInstancesOfClasses:@[[RX_TZImagePickerController class]]];
            BarItem = [UIBarButtonItem appearanceWhenContainedInInstancesOfClasses:@[[UIImagePickerController class]]];
        } else {
            tzBarItem = [UIBarButtonItem appearanceWhenContainedIn:[RX_TZImagePickerController class], nil];
...
- (void)viewDidLoad {
    ...
    self.navigationItem.title = _model.rxname;
    
    [self setBarItemTitle:tzImagePickerVc.cancelBtnTitleStr titleColor:@"#FFFFFF" target:self action:@selector(cancelButtonClick) type:NavigationBarItemTypeRight];
//    self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:tzImagePickerVc.cancelBtnTitleStr style:UIBarButtonItemStylePlain target:tzImagePickerVc action:@selector(cancelButtonClick)];
    ...
    _showTakePhotoBtn = _model.rxisCameraRoll && ((tzImagePickerVc.allowTakePicture && tzImagePickerVc.allowPickingImage) || (tzImagePickerVc.allowTakeVideo && tzImagePickerVc.allowPickingVideo));
...
- (void)setBarItemTitle:(NSString *)title titleColor:(NSString *)color target:(id)target action:(SEL)action type:(NavigationBarItemType)type {
    ...
    if (type == NavigationBarItemTypeLeft) {
        ...
        self.navigationItem.rightBarButtonItem = buttonItem;//APPMainUIColorHexString
    }
}
-(void)cancelButtonClick{
    [[NSNotificationCenter defaultCenter] postNotificationName:@"TZImagePickerVCcancelButtonClick" object:nil];
}
- (void)fetchAssetModels {
    RX_TZImagePickerController *tzImagePickerVc = (RX_TZImagePickerController *)self.navigationController;
    if (_isFirstAppear && !_model.rxmodels.count) {
        [tzImagePickerVc showProgressHUD];
    }
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        if (!tzImagePickerVc.sortAscendingByModificationDate && self->_isFirstAppear && self->_model.rxisCameraRoll) {
            [[RX_TZImageManager manager] getCameraRollAlbum:tzImagePickerVc.allowPickingVideo allowPickingImage:tzImagePickerVc.allowPickingImage needFetchAssets:YES completion:^(RX_TZAlbumModel *model) {
                self->_model = model;
                self->_models = [NSMutableArray arrayWithArray:self->_model.rxmodels];
                [self initSubviews];
            }];
        } else {
            if (self->_showTakePhotoBtn || self->_isFirstAppear) {
                [[RX_TZImageManager manager] getAssetsFromFetchResult:self->_model.rxresult completion:^(NSArray<RX_TZAssetModel *> *models) {
...
- (void)configCollectionView {
    ...
    _collectionView = [[RX_TZCollectionView alloc] initWithFrame:CGRectZero collectionViewLayout:_layout];
    ...
    if (_showTakePhotoBtn) {
        _collectionView.contentSize = CGSizeMake(self.view.tz_width, ((_model.rxcount + self.columnNumber) / self.columnNumber) * self.view.tz_width);
    } else {
        _collectionView.contentSize = CGSizeMake(self.view.tz_width, ((_model.rxcount + self.columnNumber - 1) / self.columnNumber) * self.view.tz_width);
    }
    [self.view addSubview:_collectionView];
    [_collectionView registerClass:[RX_TZAssetCell class] forCellWithReuseIdentifier:@"RX_TZAssetCell"];
    [_collectionView registerClass:[RX_TZAssetCameraCell class] forCellWithReuseIdentifier:@"RX_TZAssetCameraCell"];
}
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    RX_AssetGridThumbnailSize = CGSizeMake(cellSize.width * scale, cellSize.height * scale);
...
- (void)configBottomToolBar {
    ...
    _previewButton.titleLabel.font = SystemFontLarge;
    ...
    if (tzImagePickerVc.allowPickingOriginalPhoto) {
        ...
        _originalPhotoButton.titleLabel.font = SystemFontLarge;
        ...
        _originalPhotoLabel.font = SystemFontLarge;
    ...
    _numberLabel.font = SystemFontLarge;
...
- (void)viewDidLayoutSubviews {
    ...
    CGFloat toolBarHeight = [RX_TZCommonTools rxtz_isIPhoneX] ? 50 + (83 - 49) : 50;
    if (self.navigationController.navigationBar.isTranslucent) {
        top = naviBarHeight;
        if (!isStatusBarHidden) top += [RX_TZCommonTools rxtz_statusBarHeight];
    ...
    if (!self.navigationController.navigationBar.isHidden) {
        ...
        CGFloat navigationHeight = naviBarHeight + [RX_TZCommonTools rxtz_statusBarHeight];
    ...
    CGFloat previewWidth = [tzImagePickerVc.previewBtnTitleStr boundingRectWithSize:CGSizeMake(CGFLOAT_MAX, CGFLOAT_MAX) options:NSStringDrawingUsesFontLeading attributes:@{NSFontAttributeName:SystemFontLarge} context:nil].size.width + 2;
    ...
    if (tzImagePickerVc.allowPickingOriginalPhoto) {
        CGFloat fullImageWidth = [tzImagePickerVc.fullImageBtnTitleStr boundingRectWithSize:CGSizeMake(CGFLOAT_MAX, CGFLOAT_MAX) options:NSStringDrawingUsesFontLeading attributes:@{NSFontAttributeName:SystemFontMiddle} context:nil].size.width;
    ...
    [RX_TZImageManager manager].columnNumber = [RX_TZImageManager manager].columnNumber;
...
- (void)previewButtonClick {
    RX_TZPhotoPreviewController *photoPreviewVc = [[RX_TZPhotoPreviewController alloc] init];
...
- (void)doneButtonClick {
    ...
    if (tzImagePickerVc.onlyReturnAsset) { // not fetch image
        for (NSInteger i = 0; i < tzImagePickerVc.selectedModels.count; i++) {
            RX_TZAssetModel *model = tzImagePickerVc.selectedModels[i];
        ...
        [RX_TZImageManager manager].shouldFixOrientation = YES;
        ...
        for (NSInteger i = 0; i < tzImagePickerVc.selectedModels.count; i++) {
            ...
            [[RX_TZImageManager manager] getPhotoWithAsset:model.asset completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
                ...
                if (photo) {
                    if (![RX_TZImagePickerConfig sharedInstance].notScaleImage) {
                        photo = [[RX_TZImageManager manager] scaleImage:photo toSize:CGSizeMake(tzImagePickerVc.photoWidth, (int)(tzImagePickerVc.photoWidth * photo.size.height / photo.size.width))];
                    }
...
- (void)callDelegateMethodWithPhotos:(NSArray *)photos assets:(NSArray *)assets infoArr:(NSArray *)infoArr {
    ...
    if (tzImagePickerVc.allowPickingVideo && tzImagePickerVc.maxImagesCount == 1) {
        if ([[RX_TZImageManager manager] isVideo:[assets firstObject]]) {
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (((tzImagePickerVc.sortAscendingByModificationDate && indexPath.item >= _models.count) || (!tzImagePickerVc.sortAscendingByModificationDate && indexPath.item == 0)) && _showTakePhotoBtn) {
        RX_TZAssetCameraCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"RX_TZAssetCameraCell" forIndexPath:indexPath];
        cell.rximageView.image = tzImagePickerVc.takePictureImage;
        if ([tzImagePickerVc.takePictureImageName isEqualToString:@"takePicture80"]) {
            cell.rximageView.contentMode = UIViewContentModeCenter;
            CGFloat rgb = 223 / 255.0;
            cell.rximageView.backgroundColor = [UIColor colorWithRed:rgb green:rgb blue:rgb alpha:1.0];
        } else {
            cell.rximageView.backgroundColor = [UIColor colorWithWhite:1.000 alpha:0.500];
        }
        return cell;
    }
    // the cell dipaly photo or video / 展示照片或视频的cell
    RX_TZAssetCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"RX_TZAssetCell" forIndexPath:indexPath];
    ...
    cell.rxassetCellDidSetModelBlock = tzImagePickerVc.rxassetCellDidSetModelBlock;
    cell.assetCellDidLayoutSubviewsBlock = tzImagePickerVc.assetCellDidLayoutSubviewsBlock;
    RX_TZAssetModel *model;
    ...
    cell.didSelectPhotoBlock = ^(BOOL isSelected) {
        ...
        RX_TZImagePickerController *tzImagePickerVc = (RX_TZImagePickerController *)strongSelf.navigationController;
        // 1. cancel select / 取消选择
        if (isSelected) {
            strongCell.selectPhotoButton.selected = NO;
            model.isSelected = NO;
            NSArray *selectedModels = [NSArray arrayWithArray:tzImagePickerVc.selectedModels];
            for (RX_TZAssetModel *model_item in selectedModels) {
            ...
            [UIView showOscillatoryAnimationWithLayer:strongLayer type:RX_TZOscillatoryAnimationToSmaller];
        } else {
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    RX_TZAssetModel *model = _models[index];
    if (model.type == TZAssetModelMediaTypeVideo && !tzImagePickerVc.allowPickingMultipleVideo) {
        if (tzImagePickerVc.selectedModels.count > 0) {
            ...
            RX_TZVideoPlayerController *videoPlayerVc = [[RX_TZVideoPlayerController alloc] init];
        ...
        if (tzImagePickerVc.selectedModels.count > 0) {
            ...
            RX_TZGifPhotoPreviewController *gifPreviewVc = [[RX_TZGifPhotoPreviewController alloc] init];
...
- (void)takePhoto {
    ...
    if ((authStatus == AVAuthorizationStatusRestricted || authStatus ==AVAuthorizationStatusDenied)) {
        ...
        NSDictionary *infoDict = [RX_TZCommonTools rxtz_getInfoDictionary];
        // 无权限 做一个友好的提示
...
- (void)pushImagePickerController {
    ...
    if (tzImagePickerVc.allowCameraLocation) {
        __weak typeof(self) weakSelf = self;
        [[RX_TZLocationManager manager] startLocationWithSuccessBlock:^(NSArray<CLLocation *> *locations) {
            __strong typeof(weakSelf) strongSelf = weakSelf;
...
- (void)pushPhotoPrevireViewController:(RX_TZPhotoPreviewController *)photoPreviewVc {
    [self pushPhotoPrevireViewController:photoPreviewVc needCheckSelectedModels:NO];
}

- (void)pushPhotoPrevireViewController:(RX_TZPhotoPreviewController *)photoPreviewVc needCheckSelectedModels:(BOOL)needCheckSelectedModels {
    __weak typeof(self) weakSelf = self;
...
- (void)getSelectedPhotoBytes {
    ...
    if ([[RX_TZImagePickerConfig sharedInstance].preferredLanguage isEqualToString:@"vi"] && self.view.tz_width <= 320) {
        return;
    }
    RX_TZImagePickerController *imagePickerVc = (RX_TZImagePickerController *)self.navigationController;
    [[RX_TZImageManager manager] getPhotosBytesWithArray:imagePickerVc.selectedModels completion:^(NSString *totalBytes) {
        /// eagle 视频类型的话，返回长度为零，直接不显示
        if ([totalBytes floatValue] > 0) {
...
- (void)checkSelectedModels {
    ...
    RX_TZImagePickerController *tzImagePickerVc = (RX_TZImagePickerController *)self.navigationController;
    for (RX_TZAssetModel *model in tzImagePickerVc.selectedModels) {
        [selectedAssets addObject:model.asset];
    }
    for (RX_TZAssetModel *model in _models) {
        model.isSelected = NO;
...
- (void)imagePickerController:(UIImagePickerController*)picker didFinishPickingMediaWithInfo:(NSDictionary *)info {
    ...
    if ([type isEqualToString:@"public.image"]) {
        ...
        if (photo) {
            [[RX_TZImageManager manager] savePhotoWithImage:photo location:self.location completion:^(PHAsset *asset, NSError *error){
                if (!error) {
        ...
        if (videoUrl) {
            [[RX_TZImageManager manager] saveVideoWithUrl:videoUrl location:self.location completion:^(PHAsset *asset, NSError *error) {
                if (!error) {
                    [self addPHAsset:asset];
                }
            }];
...
- (void)addPHAsset:(PHAsset *)asset {
    RX_TZAssetModel *assetModel = [[RX_TZImageManager manager] createModelWithAsset:asset];
    RX_TZImagePickerController *tzImagePickerVc = (RX_TZImagePickerController *)self.navigationController;
    [tzImagePickerVc hideProgressHUD];
    if (tzImagePickerVc.sortAscendingByModificationDate) {
...
- (void)resetCachedAssets {
    [[RX_TZImageManager manager].cachingImageManager stopCachingImagesForAllAssets];
...
- (void)updateCachedAssets {
    ...
    if (delta > CGRectGetHeight(_collectionView.bounds) / 3.0f) {
        ...
        [[RX_TZImageManager manager].cachingImageManager startCachingImagesForAssets:assetsToStartCaching
                                                                       targetSize:RX_AssetGridThumbnailSize
                                                                      contentMode:PHImageContentModeAspectFill
                                                                          options:nil];
        [[RX_TZImageManager manager].cachingImageManager stopCachingImagesForAssets:assetsToStopCaching
                                                                      targetSize:RX_AssetGridThumbnailSize
                                                                     contentMode:PHImageContentModeAspectFill
                                                                         options:nil];
...
- (NSArray *)assetsAtIndexPaths:(NSArray *)indexPaths {
    ...
    for (NSIndexPath *indexPath in indexPaths) {
        if (indexPath.item < _models.count) {
            RX_TZAssetModel *model = _models[indexPath.item];
            [assets addObject:model.asset];
        }
    }
...
@implementation RX_TZCollectionView
