...
#import "RXMenuDefaultView.h"
#import "RXMenuController.h"
#import "RXMenuViewContainer.h"
#import "RXAdjustButton.h"
@interface RXMenuItemDefaultView:RXAdjustButton
@property (nonatomic, strong) UIColor  *highlightedColor;
@property (nonatomic, strong) UIImageView  *effectView;
@end
@implementation RXMenuItemDefaultView
...
@interface RXMenuDefaultView()
@property (nonatomic, strong) UIView * contentView;
@property (nonatomic, weak) RXMenuViewContainer * container;
...
@implementation RXMenuDefaultView
...
+ (instancetype)defaultView:(RXMenuViewContainer*)container WithMenuItems:(NSArray<RXMenuItem*>*)menuItems MaxSize:(CGSize)maxSize arrowSize:(CGSize)arrowSize AnchorPoint:(CGPoint)anchorPoint {
    RXMenuDefaultView *defaultView = [[RXMenuDefaultView alloc] initView:container WithMenuItems:menuItems MaxSize:maxSize arrowSize:arrowSize AnchorPoint:anchorPoint];
    return defaultView;
}

- (instancetype)initView:(RXMenuViewContainer*)container WithMenuItems:(NSArray<RXMenuItem*>*)menuItems MaxSize:(CGSize)maxSize arrowSize:(CGSize)arrowSize AnchorPoint:(CGPoint)anchorPoint {
    if (self = [super init]) {
...
- (void)dealloc {
   // NSLog(@"RXMenuDefaultView dealloc~");
}
...
- (void)processLineWithMidX:(CGFloat)midX direction:(RXMenuControllerArrowDirection)direction {
    ...
    [self.lines enumerateObjectsUsingBlock:^(UIView* line, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if (direction == RXMenuControllerArrowDown) {
...
- (void)setCorrectDirection:(RXMenuControllerArrowDirection)CorrectDirection {
    _CorrectDirection = CorrectDirection;
    if (CorrectDirection == RXMenuControllerArrowDown) {
...
- (void)layoutMenuViews:(BOOL)needResetLayout {
    ...
    [array enumerateObjectsUsingBlock:^(RXMenuItem * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if (obj.image && (self.container.imagePosition == RXAdjustButtonIMGPositionLeft || self.container.imagePosition == RXAdjustButtonIMGPositionRight)) {
             RXMenuItemDefaultView *test = [RXMenuItemDefaultView buttonWithType:UIButtonTypeCustom];
        ...
        if ((totalWidth > maxWidth) && (totalWidth - maxWidth) > kTitleMargin) {
            ...
            index = idx;
            if (idx <= (itemsCount-1)) {
                RXMenuItemDefaultView *more = [RXMenuItemDefaultView buttonWithType:UIButtonTypeCustom];
            ...
            *stop = YES;
            ...
            RXMenuItemDefaultView *item = [RXMenuItemDefaultView buttonWithType:UIButtonTypeCustom];
...
- (void)layoutMoreMenuViews {
    ...
    if (self.totalCount != 0) {
        ...
        [self.menuItems enumerateObjectsUsingBlock:^(RXMenuItem * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
    ...
    // moreLeft
    {
        RXMenuItemDefaultView *moreleft = [RXMenuItemDefaultView buttonWithType:UIButtonTypeCustom];
    ...
    [newMenuArray enumerateObjectsUsingBlock:^(RXMenuItem *  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        CGFloat maxWidth = (self.maxSize.width-kMoreWidth*2);
    ...
    // moreRight
    {
        RXMenuItemDefaultView *moreRight = [RXMenuItemDefaultView buttonWithType:UIButtonTypeCustom];
...
- (CGSize)calculateTextSize:(RXMenuItem*)obj maxWidth:(CGFloat)maxWidth {
