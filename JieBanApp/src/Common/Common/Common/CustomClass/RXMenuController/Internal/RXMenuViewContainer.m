...
#define RXMenuStatusBarHeight  [UIApplication sharedApplication].statusBarFrame.size.height
#define RXMenuScreenHeight [UIScreen mainScreen].bounds.size.height
#define RXMenuScreenWidth [UIScreen mainScreen].bounds.size.width
static inline CGPoint GMenuGetXCenter(CGRect rect) {
    return CGPointMake(CGRectGetMidX(rect), rect.origin.y);
}


@interface RXMenuViewContainer() {
    CGFloat _arrowMidx;
}
...
@implementation RXMenuViewContainer
...
- (void)layoutSubviews
{
    ...
    CGRect roundedRect = CGRectMake(0, (self.CorrectDirection == RXMenuControllerArrowUp ? 0 : arrowSize.height), CGRectGetWidth(self.bounds) , CGRectGetHeight(self.bounds) - arrowSize.height);
    CGRect menuRect = self.menuView.frame;
    menuRect.origin.y = (self.CorrectDirection == RXMenuControllerArrowUp ? 0 : arrowSize.height);
    ...
    if (self.CorrectDirection == RXMenuControllerArrowUp) {
        //始画三角形，箭头向下
    ...
    if (self.CorrectDirection == RXMenuControllerArrowDown) {
        // 箭头向上
...
- (void)setImagePosition:(RXAdjustButtonIMGPosition)imagePosition {
...
- (void)processMenuFrame {
    ...
    CGRect rect = [self.targetView convertRect:self.targetRect toView:[RXMenuEffectsWindow sharedWindow]];
   
    if (self.menuView.superview) {
        [self.menuView removeFromSuperview];
    }
    self.menuView = [RXMenuDefaultView defaultView:self WithMenuItems:_menuItems MaxSize:CGSizeMake(_maxMenuViewWidth-_menuEdgeInset.left-_menuEdgeInset.right, _menuViewHeight) arrowSize:_arrowSize AnchorPoint:CGPointZero];
    ...
    if (self.AnchorPoint.x < RXMenuScreenWidth/2) {
        ...
        if ((self.AnchorPoint.x + menuRect.size.width/2)>(RXMenuScreenWidth-self.menuEdgeInset.right)) {
            menuRect.origin.x = (RXMenuScreenWidth - self.menuEdgeInset.right-menuRect.size.width);
        } else {
            menuRect.origin.x =(self.AnchorPoint.x - menuRect.size.width/2);
        }
    }
    ...
    switch (self.CorrectDirection) {
        case RXMenuControllerArrowUp:
        {
            menuRect.origin.y = self.AnchorPoint.y-menuRect.size.height;
        }
            break;
        case RXMenuControllerArrowDown:
        {
    ...
    if (!CGRectEqualToRect(self.frame, menuRect)) {
        [[NSNotificationCenter defaultCenter] postNotificationName:RXMenuControllerMenuFrameDidChangeNotification object:nil];
    }
    self.frame = menuRect;
...
- (CGPoint)calculateAnchorPoint:(CGRect)targetRect menuViewSize:(CGSize)size {
    ...
    if (centerPoint.x < 0 || centerPoint.x > RXMenuScreenWidth) {
        centerPoint.x = RXMenuScreenWidth/2;
    }
    if (centerPoint.y < 0 || centerPoint.y > RXMenuScreenHeight) {
        centerPoint.y = RXMenuScreenHeight/2;
    }
    CGPoint targetPoint = CGPointMake(centerPoint.x, centerPoint.y);

    CGSize tempSize = size;
    switch (self.arrowDirection) {
        case RXMenuControllerArrowDefault:
            if ((targetPoint.y - self.arrowMargin - tempSize.height) > RXMenuStatusBarHeight) {
                self.CorrectDirection = RXMenuControllerArrowUp;
                targetPoint.y -= self.arrowMargin;
            } else {
                self.CorrectDirection = RXMenuControllerArrowDown;
        ...
        case RXMenuControllerArrowUp:
            self.CorrectDirection = RXMenuControllerArrowUp;
            if ((targetPoint.y - self.arrowMargin - tempSize.height) < RXMenuStatusBarHeight) {
                targetPoint.y += (RXMenuStatusBarHeight - (targetPoint.y - self.arrowMargin - tempSize.height));
            } else {
                targetPoint.y  -= self.arrowMargin;
            }
            break;
