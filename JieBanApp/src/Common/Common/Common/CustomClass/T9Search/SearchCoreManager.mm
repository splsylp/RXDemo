...
- (void)AddContact:(NSNumber*)localID name:(NSString*)name phone:(NSDictionary*)phoneDic {
    //将联系人的号码用分隔符拼接添加到搜索,不直接用Array,为了优化号码搜索(KMP复杂度M+N)
    
    NSMutableString *phoneStr = [[NSMutableString alloc] init];
    for (NSInteger i = 0; i < [phoneDic count]; i ++) {
        NSString *phone = [phoneDic.allValues objectAtIndex:i];
