...
#import "HYTBaseData.h"
#import "WFMessageBody.h"

@interface RXSportMeetData : HYTBaseData
...
//插入运动会和同学圈数据
+(void)insertSportMessageData:(KXJson *)json;

//获取运动会列表
+(NSMutableArray *)getAllSportMessageType:(NSString *)type;

+(void)insertjsonString:(SportMeetReceiveModel *)model;
...
//单批入库
+(void)insertSportBrowsejson:(KXJson *)browseJson;
//同事圈单批入库
