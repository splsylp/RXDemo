...
/*
 经过测试，比起xib的方式，把RX_TZAssetCell改用纯代码的方式来写，滑动帧数明显提高了（约提高10帧左右）
...
@class RX_TZAlbumCell, RX_TZAssetCell;
@protocol RX_TZImagePickerControllerDelegate;
@interface RX_TZImagePickerController : UINavigationController
...
@property (nonatomic, strong) NSMutableArray<RX_TZAssetModel *> *selectedModels;
...
// You can get original photo with asset, by the method [[RX_TZImageManager manager] getOriginalPhotoWithAsset:completion:].
...
@property (nonatomic, weak) id<RX_TZImagePickerControllerDelegate> pickerDelegate;

@end


@protocol RX_TZImagePickerControllerDelegate <NSObject>
@optional
// The picker should dismiss itself; when it dismissed these handle will be called.
// You can also set autoDismiss to NO, then the picker don't dismiss itself.
// If isOriginalPhoto is YES, user picked the original photo.
...
// 你可以通过一个asset获得原图，通过这个方法：[[RX_TZImageManager manager] getOriginalPhotoWithAsset:completion:]
// photos数组里的UIImage对象，默认是828像素宽，你可以通过设置photoWidth属性的值来改变它
- (void)imagePickerController:(RX_TZImagePickerController *)picker didFinishPickingPhotos:(NSArray<UIImage *> *)photos sourceAssets:(NSArray *)assets isSelectOriginalPhoto:(BOOL)isSelectOriginalPhoto;
...
//- (void)imagePickerControllerDidCancel:(RX_TZImagePickerController *)picker __attribute__((deprecated("Use -tz_imagePickerControllerDidCancel:.")));
- (void)tz_imagePickerControllerDidCancel:(RX_TZImagePickerController *)picker;
...
@interface RX_TZAlbumPickerController : UIViewController
...
@interface RX_TZCommonTools : NSObject
+ (BOOL)rxtz_isIPhoneX;
...
@interface RX_TZImagePickerConfig : NSObject
